...
Note que este chunk exibe tanto o código quanto o resultado do gráfico. Se estivéssemos escrevendo um relatório para um cliente, a parte do código deveria se eliminada. Felizmente, o `knitr` permite que coloquemos a opção `echo=FALSE` na definição do chunk e, assim, apenas o gráfico é produzido:

```{r CarsPlot_3, echo=FALSE}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)")
```

Se colocarmos a opção `eval=FALSE`, apenas o código é exibido, sem que o gráfico seja plotado:

```{r CarsPlot_4, eval=FALSE}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)")
```

Se quisermos, podemos colocar um label na Figura, para que ela possa ser referenciada posteriormente:

```{r CarsPlot_5, echo=FALSE, fig.cap="\\label{CarsPlot_5} Gráfico de dispersão entre a distância necessária para frear completamente um carro (em pés) e sua velocidade (em milhas por hora)."}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)")
```

Veja como consigo referenciar a Figura \ref{CarsPlot_5} como se eu estivesse utilizando LaTeX. Eu posso inclusive fazê-la novamente em outra cor, para mostrar que a numeração é atualizada como no LaTeX tradicional:

```{r CarsPlot_6, echo=FALSE, fig.cap="\\label{CarsPlot_6} Gráfico de dispersão com os pontos em vermelho entre a distância necessária para frear completamente um carro (em pés) e sua velocidade (em milhas por hora)."}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)", col="red", pch=16)
```

Portanto, a Figura \ref{CarsPlot_6} mostra aquilo que foi prometido no parágrafo anterior. Note que estamos de fato utilizando LaTeX, pois a figura na página nnao ficou onde desejávamos!
...
Portanto, a Tabela \ref{CarsLM_4} ficou exatamente do jeito que imaginávamos. Agora podemos fazer o gráfico final da nossa análise, plotando os dados em conjunto com a reta ajustada. Este resultado se encontra na Figura \ref{CarsPlot_7}.

```{r CarsPlot_7, fig.cap="\\label{CarsPlot_7} Resultado do ajuste linear entre a distância necessária para frear completamente um carro (em pés) e sua velocidade (em milhas por hora)."}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)")
abline(ajuste)
```

Caso a Figura \ref{CarsPlot_7} tenha ficado muito grande, podemos alterar as suas dimensões, como no caso da Figura \ref{CarsPlot_8}.

```{r CarsPlot_8, fig.height=8, fig.width=5, fig.cap="\\label{CarsPlot_8} Resultado do ajuste linear entre a distância necessária para frear completamente um carro (em pés) e sua velocidade (em milhas por hora), com as dimensões alteradas."}
plot(dist ~ speed, data=cars, xlab="Velocidade (mph)", ylab="Distância (pés)")
abline(ajuste)
```
...
Para que este relatório fique completo, só falta a análise dos resíduos do ajuste realizado. A Figura \ref{CarsPlot_9} exibe este resultado.

```{r CarsPlot_9, fig.height=8, fig.cap="\\label{CarsPlot_9} Gráficos de diagnóstico do ajuste linear entre a distância necessária para frear completamente um carro (em pés) e sua velocidade (em milhas por hora)."}
par(mfrow=c(2,2))
plot(ajuste)
```
